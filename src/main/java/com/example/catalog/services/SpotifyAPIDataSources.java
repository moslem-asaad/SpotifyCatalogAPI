...
public class SpotifyAPIDataSources implements DataSourceService{
    ...
    public String isValidToken() throws IOException {
    ...
    public ResponseEntity<Map> callAPI(String accessToken,String url,HttpMethod method){
    ...
    public ResponseEntity<Void> callAPI2(String accessToken,String url,HttpMethod method){
        HttpHeaders headers = new HttpHeaders();
        headers.set("Authorization", "Bearer " + accessToken);
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<String> entity = new HttpEntity<>(headers);
